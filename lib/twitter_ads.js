...
function TwitterAds(options) {
  ...
  self._validateConfigOrThrow = function(config) {
    ...
    constants.CONFIGURABLE_KEYS.forEach(function(k) {
      ...
      else if (constants.REQUIRED_KEYS.indexOf(k) > -1) throw new Error(util.format('Twitter-Ads: Config must include `%s`.', k));
    });
  ...
  self._makeRequest = function(type, url, params, cb) {
    if (!url) throw new Error('Twitter-Ads: URL must be provided when making an API call.');
    if (typeof params == 'function') {
      cb = params;
      params = {};
    }
    if (!cb) throw new Error('Twitter-Ads: Callback must be provided when making an API call.');
...
TwitterAds.prototype.setOptions = function(options) {
  this._validateConfigOrThrow(options);
  return this;
};

TwitterAds.prototype.getOptions = function() { return this.config; };
