  ...
  self._uploadTonChunks = function(fd, chunkSize, url, params, cb) {
    ...
    while (totalBytesRead < totalBytes) {
      ...
      request.put(null, {
        ...
        if (err) {
          return requestFuture.throw(err);
        }
        
        if (resp.statusCode !== 308 && resp.statusCode !== 201)
          requestFuture.throw(new Error('Twitter did not accept chunk upload, Should return status code 308 or 201 but instead returned: ' + resp.statusCode));
      
        if (resp.statusCode === 201)
          finalLocation = constants.TON_API_HOST + self.config.ton_api_version + resp.headers.location.substring(1);
        
        requestFuture.return();
      });
      
      var result = requestFuture.wait();
      if (result instanceof Error)
        return uploadFuture.throw(result);
    ...
    fs.closeSync(fd);
    uploadFuture.return(finalLocation);
    
    var result = uploadFuture.wait();
    if (result instanceof Error) {
      fs.closeSync(fd);
      return cb(result);
    }
