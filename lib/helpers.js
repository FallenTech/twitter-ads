...
helpers.parseResponse = function(body, resp, cb) {
  var parsedBody;
  if (body && resp.headers['content-type'].indexOf('application/json') > -1) parsedBody = helpers.tryJSONParse(body);
  if (constants.STATUS_CODES_TO_ABORT_ON.indexOf(resp.statusCode) > -1 || (parsedBody && parsedBody.errors && parsedBody.errors.length)) {
    ...
    return cb(err, parsedBody || body, resp);
  }
  return cb(null, parsedBody || body, resp);
};
